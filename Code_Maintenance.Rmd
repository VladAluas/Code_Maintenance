...
We will take a look at these two methods in more detailf below.
...
# We will use points over the boxplots to show where each observation falls exaclty
  geom_point() +

# The text is optional but a nice to have, so we will know which car is which
  geom_text(mapping = aes(label = car_names),
  
# We will add some adjustements to the geom_text so the writing will not overlap the points
            hjust = 0,
            nudge_x = 0.05,
...
As you can see, it is much easier for someone to look at the second example and undestand why I have added the elements that I did and check if some of them are useful or not in the code, or the final product. Some of you might disagree with my choices, however, you know why I chose this approach and you can base your argument on that.
...
Remember when commenting on code, it's important to say **WHY** you are using an aproach or the other, not just describe **WHAT** the code does. That should be self evident.
...
As you can guess from the names it is preferrable to have **DRY** code as opposed to **WET**. Usually this means that **DRY** code is not repeated over and over again, while **WET** code is found in your project more than 3-4 times. 
